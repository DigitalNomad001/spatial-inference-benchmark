...
def generate_neighbor_radio_question(triple, none_prob=0.2):
    subj, pred, obj = triple
    if pred.replace('adjacent to and ', '') not in CYCLE:
        return None, None
    ...
    for step in range(1, 5):
        for offset in (step, -step):
            cand = CYCLE[(i + offset) % 8]
            if cand != base and cand not in neighbors:
                neighbors.append(cand)
    if pred.startswith('adjacent to and '):
        neighbors = ['adjacent to and ' + x for x in neighbors]

    if random.random() < none_prob:
        # 正确答案是 e:a-d 全用近邻干扰,真值不出现
        options = neighbors[:4]
        if len(options) < 4:
            return None, None
        ...
        # 正确答案在 a-d:真值 + 3 个近邻干扰
        distractors = neighbors[:3]
        if len(distractors) < 3:
            return None, None
        options = [pred] + distractors
        random.shuffle(options)
        truth = POS_TO_LETTER[options.index(pred)]
        options = options + ["none of the above"]

    question = (
        f"Question: Select exactly one option (a-e) that best describes the relationship "
        f"of {subj} in relation to {obj} in terms of geography. "
...
def main():
    ...
    parser.add_argument('-radio_neighbor_output', default='queries_radio_neighbor.csv', help='Output path for radio neighbour questions')
    ...
    #     q, t = generate_neighbor_yesno_question(triple, steps=args.steps)   
    #     if q is not None:
    #         neighbor_questions.append((q, t))                              
    ...
    # n_yes = sum(1 for q, t in neighbor_questions if t == "yes")            
    # n_no = len(neighbor_questions) - n_yes
    # print(f"[neighbor] wrote {len(neighbor_questions)} questions -> {args.neighbor_output}"
    #       f"  (yes={n_yes}, no={n_no})")

    # ===== 新增:生成 radio 近邻题 =====
    radio_neighbor_questions = []
    for triple in sampled_triples:
        q, t = generate_neighbor_radio_question(triple)
        if q is not None:
            radio_neighbor_questions.append((q, t))

    df_radio = pd.DataFrame(radio_neighbor_questions, columns=["query", "truth"])
    df_radio.to_csv(args.radio_neighbor_output, index=False)
    print(f"[radio neighbor] wrote {len(radio_neighbor_questions)} questions -> {args.radio_neighbor_output}")
